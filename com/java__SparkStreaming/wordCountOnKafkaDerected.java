 ...
 */
public class wordCountOnKafkaDerected {
    ...
    public static void main(String[] args) {
         ...
         *  此时灭有Receiver，SparkStreaming应用程序只是按照时间间隔监控目录下每个Batch
         *  新增的内容作为RDD的数据来源产生原始的RDD
         */
        //JavaDStream lines = jssc.textFileStream("hdfs://Master:9000/sparkStreaming/data");//监控新增的文件。
        //在本案例中第一个参数是StreamingContext实例；
        // 第二个参数是Zookeeper集群信息（接受Kfaka数据的时候会从Zookeeper中获得Offset等元数据信息）；
        ...
        JavaPairReceiverInputDStream<String,String> lines = KafkaUtils.createStream(jssc,"master:2181,slave1:2181","MyFirstCousumerGroup",topicConsumerConcurrency);
        /**
    ...
    private static JavaStreamingContext createContext(String checkpointDirectory) {
        ...
        SparkConf sparkConf = new SparkConf().setAppName("wordCountOnKafkaReceiver");
