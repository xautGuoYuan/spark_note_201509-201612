 ...
 * 用户 推荐物品 评分
 */
case class UserRecomm(val userid:String, val itemid:String, val pref:Double) extends Serializable

/**
 * 相似度
 * 物品 物品 相似度
 */
case class ItemSimi(val itemid1:String, val itemid2:String,val similar:Double) extends Serializable
...
object ItemSimilarity{
   ...
   *
   */
  def CosineSimilarity(user_rdd: RDD[ItemPref]) = {
    // 0 数据做准备
    val user_rdd1 = user_rdd.map(f => (f.userid, f.itemid, f.pref))
